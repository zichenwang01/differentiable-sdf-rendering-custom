...
def optimize_suzanne(scene_config, mts_args, ref_image_paths,
                   output_dir, config, write_ldr_images=True):
    """Main function that optmizes the geometry by looking at the shadow"""
    ...
    center_image_dir = join(output_dir, 'center')
    os.makedirs(center_image_dir, exist_ok=True)
    ...
    ref_image_paths = ['/home/zw336/IR/differentiable-sdf-rendering/outputs/renders/suzanne_bottom/exp_suzanne_bottom/sdf_direct_reparam/ref/ref-00.exr', '/home/zw336/IR/differentiable-sdf-rendering/outputs/renders/suzanne_bottom/exp_suzanne_bottom/sdf_direct_reparam/ref/ref-01.exr', '/home/zw336/IR/differentiable-sdf-rendering/outputs/renders/suzanne_bottom/exp_suzanne_bottom/sdf_direct_reparam/ref/ref-02.exr', '/home/zw336/IR/differentiable-sdf-rendering/outputs/renders/suzanne_bottom/exp_suzanne_bottom/sdf_direct_reparam/ref/ref-03.exr', '/home/zw336/IR/differentiable-sdf-rendering/outputs/renders/suzanne_bottom/exp_suzanne_bottom/sdf_direct_reparam/ref/ref-04.exr', '/home/zw336/IR/differentiable-sdf-rendering/outputs/renders/suzanne_bottom/exp_suzanne_bottom/sdf_direct_reparam/ref/ref-05.exr']
    print("ref_image_paths: ", ref_image_paths)
    ...
    center_scene = mi.load_file(
        join(SCENE_DIR, scene_config.scene, 'scene_center.xml'),
    ...
    center_sensor = mi.load_dict({
        'type': 'perspective', 'fov': 45,
        'sampler': {'type': 'independent'},
        'film': {
            'type': 'hdrfilm',
            'width': scene_config.resx, 'height': scene_config.resy,
    ...
    scenes = []
    for i in range(6):
        scene_name = join(SCENE_DIR, scene_config.scene, f'scene{i+1}.xml')
        scene  = mi.load_file(
            scene_name, s=s, L=L,
            shape_file='dummysdf.xml', # load initial sphere
            sdf_filename=join(SCENE_DIR, 'sdfs', 'bunny_res=256.sdf'),
    ...
    sdf_scene = scenes[0]
    scenes[1].integrator().sdf = sdf_scene.integrator().sdf
    scenes[2].integrator().sdf = sdf_scene.integrator().sdf
    scenes[3].integrator().sdf = sdf_scene.integrator().sdf
    scenes[4].integrator().sdf = sdf_scene.integrator().sdf
    scenes[5].integrator().sdf = sdf_scene.integrator().sdf
    center_scene.integrator().sdf = sdf_scene.integrator().sdf
    ...
    with dr.suspend_grad():
        for idx, sensor in enumerate(scene_config.sensors):
            img = mi.render(scenes[idx], sensor=sensor, seed=idx,
    ...
    try:
        ...
        for i in pbar:
            ...
            with dr.suspend_grad():
                img = mi.render(
                    center_scene, sensor=center_sensor, seed=seed,
                    spp=config.spp * config.primal_spp_mult,
                    spp_grad=config.spp
                )
                bitmap = mi.Bitmap(img)
                mi.util.write_bitmap(join(center_image_dir, f'center-{i:04d}.png'), bitmap)
            ...
            for idx, sensor in scene_config.get_sensor_iterator(i):
                ...
                # start = time.time()
                img = mi.render(
                    scenes[idx], params=params, sensor=sensor, seed=seed, 
                    spp=config.spp * config.primal_spp_mult,
